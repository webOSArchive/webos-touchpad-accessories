...
| 11 | **bluetooth-shim installed** (2026-07-27). **TouchPad Go, 2026-08-31: deliberately reverted to 1.1.0** (registry 1.1.0, `jail_pdk.conf` 4014 B with the bind-mount patch) so the Preware 1.1.0→1.2.0 upgrade can be tested on real hardware — **PDK games are expected to be broken in this state**. Stock safety copies at `/var/btgamepad-STOCK-jail_pdk.conf.SAFETY` and `/var/btgamepad-STOCK-bluetooth.upstart.SAFETY`; the 1.1.0 ipk is on-device at `/media/internal/.developer/btgamepad_110.ipk`. To un-break without 1.2.0: `cp /etc/jail_pdk.conf.btshim-orig /etc/jail_pdk.conf` + reboot. See row 16): `/usr/lib/libpmbtgamepad.so`; `/etc/event.d/bluetooth` replaced with LD_PRELOAD wrapper (also forces sysrq=0 at BT start); original job backed up at `/etc/bluetooth.upstart.btshim-orig` (deliberately OUTSIDE /etc/event.d) | Yes | `cp /etc/bluetooth.upstart.btshim-orig /etc/event.d/bluetooth; rm /usr/lib/libpmbtgamepad.so` + reboot, or `bluetooth-shim/scripts/undeploy.sh` |
...
| 16 | `/etc/jail_pdk.conf` patched so jailed PDK apps can read the gamepad node; backup at `/etc/jail_pdk.conf.btshim-orig`. **As of btgamepad 1.2.0 this is a `mknod` block** (`/dev/input/event3`-`event9`, char 13:67-13:73) bracketed by `# >>> btgamepad begin` / `# <<< btgamepad end` markers, applied on **opal only**. 1.1.0 used `mkdir /dev/input` + `mount ro /dev/input`, a real bind mount of the host's `/dev/input` - it blacks out PDK games on the Go and lets a jail `rm -rf` delete the device's real input nodes. See `bluetooth-shim/docs/PDK-GRAPHICS-REGRESSION.md`. | Yes | `bluetooth-shim/scripts/undeploy.sh`, or restore the backup |
